...
\usepackage{graphicx}
\usepackage{hyperref} % Required for clickable email links
\usepackage{enumitem}
\usepackage{booktabs}
\usepackage{xcolor}
\usepackage{tabularx}
...
\date{April 9, 2025} % Use specific date or \today

\begin{document}

\maketitle

% Updated Author Information section based on Search Result [4]
...
\begin{itemize}[noitemsep, topsep=0pt] % Reduced spacing for inner lists
...
% Adding sections required by the Portfolio Assignment
...
% Placeholder text according to assignment guidelines. Adjust if necessary.
